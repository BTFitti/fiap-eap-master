...
public class AlunoDAOImpl extends HibernateGenericDAO<Aluno, Long> {
	...
	public List<Aluno> listarFetchEndereco(String rua, EntityManager entityManager) {
		...
		Predicate predicate = builder.like(builder.upper(endereco.get("rua")), "%" + rua.toUpperCase() + "%");
		criteriaQuery.select(aluno);
	...
	public List<AlunoCidadeDTO> listarAlunoCidadeCriteria(EntityManager entityManager) {
		CriteriaBuilder builder = entityManager.getCriteriaBuilder();
		...
		criteriaQuery.select(builder.construct(AlunoCidadeDTO.class,aluno.get("nome"),endereco.get("cidade")));
