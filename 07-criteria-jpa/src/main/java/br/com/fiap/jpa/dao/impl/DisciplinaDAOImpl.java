...
public class DisciplinaDAOImpl extends HibernateGenericDAO<Disciplina, Long> {
	...
	public List<Disciplina> listarPorCargaHoraria(Integer cargaHoraria, EntityManager entityManager) {
	...
	public List<Disciplina> pesquisar(String nome, Integer cargaHoraria, LocalDateTime dataCadastro,
		...
		if (dataCadastro != null) {
			Predicate predicate = builder.greaterThanOrEqualTo(disciplina.get("dataCadastro"), dataCadastro);
			predicates.add(predicate);
		}
	...
	public List<String> listarConcatenado(EntityManager entityManager) {
		...
		Expression<String> expressao = builder.concat(builder.concat(disciplina.get("nome"), "-"),
	...
	public List<Disciplina> buscarPorNomeIgnoreCase(String nome, EntityManager entityManager) {
		...
		Predicate predicate = builder.like(builder.upper(disciplina.get("nome")), "%" + nome.toUpperCase() + "%");
	...
	public List<String> listarTudo(String nome, EntityManager entityManager) {
		CriteriaBuilder builder = entityManager.getCriteriaBuilder();
		CriteriaQuery<String> criteriaQuery = builder.createQuery(String.class);
		...
		criteriaQuery.select(expressao);
		criteriaQuery.orderBy(orderNome);
		criteriaQuery.where(predicate);
		TypedQuery<String> consulta = entityManager.createQuery(criteriaQuery);
