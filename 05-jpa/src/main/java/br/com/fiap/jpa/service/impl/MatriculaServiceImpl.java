...
public class MatriculaServiceImpl extends GenericService<Matricula, Long> {
	...
	private MatriculaServiceImpl() {
		this.matriculaDAO = MatriculaDAOImpl.getInstance();
		this.alunoDAO = AlunoDAOImpl.getInstance();
		this.cursoDAO = CursoDAOImpl.getInstance();
	}
	...
	public void inserir(Matricula instance) {
		try {
			...
			instance.setCurso(cursoDAO.obterPorId(instance.getAluno().getId(), getEntityManager()));
